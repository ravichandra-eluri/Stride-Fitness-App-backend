...
// MealRecipe returns ingredients + brief steps for a single meal name.
// Stateless: result is generated by Claude on demand and not cached server-side
// (small responses, infrequent taps; not worth a DB table).
func MealRecipe(d Deps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var body struct {
			Name string `json:"name"`
		}
		if err := decode(r, &body); err != nil || len(body.Name) < 2 || len(body.Name) > 120 {
			respondErr(w, 400, "name (2-120 chars) required")
			return
		}

		recipe, err := d.aiClient().GenerateMealRecipe(r.Context(), body.Name)
		if err != nil {
			log.Printf("[recipe] name=%q: %v", body.Name, err)
			respondErr(w, 502, "recipe generation failed")
			return
		}
		respond(w, 200, recipe)
	}
}
